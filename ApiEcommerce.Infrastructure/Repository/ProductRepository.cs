...
public class ProductRepository : IProductRepository
{
    ...
    public bool BuyProduct(string name, int cantidad)
    {
        if (string.IsNullOrWhiteSpace(name) || cantidad <= 0)
        {
            return false;
        }
    ...
    public ICollection<Product> GetProductForCategory(int IdCategory)
    {
        if(IdCategory <= 0)
        {
        ...
        return _db.Products.Include(p=> p.Category).Where(P=> P.CategoryId == IdCategory).OrderBy(p=> p.Name).ToList();
    }
    ...
    {
        return _db.Products.OrderBy(p=> p.Id ).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToList();
    }
    ...
    public ICollection<Product> SearchProducts(string searchTerm)
    {
        ...
        if (!string.IsNullOrEmpty(searchTerm))
        {
            ...
            query= query.Where(
                p=> p.Name.ToLower().Trim().Contains(searchTermLower) || p.Description.ToLower().Trim().Contains( searchTermLower));
        } 
